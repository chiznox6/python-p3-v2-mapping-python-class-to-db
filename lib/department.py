...
class Department:
    ...
    def save(self):
        """Save the Department instance to the db and assign its id."""
    ...
    def update(self):
        """Update the db row to match the instance’s current values."""
        sql = "UPDATE departments SET name = ?, location = ? WHERE id = ?"
        CURSOR.execute(sql, (self.name, self.location, self.id))
        CONN.commit()

    def delete(self):
        """Delete the db row corresponding to this Department instance."""
